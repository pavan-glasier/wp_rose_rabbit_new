...
class pisol_corw_cancel_request{
    ...
    static function recordCancellationRequest($order_id, $reason, $predefined_reason){
        $order = wc_get_order($order_id);
        update_post_meta($order_id, 'order_cancel_reason', $reason);
        update_post_meta($order_id, 'cancellation_date', current_time('Y/m/d H:i'));
        update_post_meta($order_id, 'predefined_reason', $predefined_reason);
